...
pub use families::*;
pub use real_bugs::{RealBugId, RealBugInfo};
...
pub enum EntrySource {
    ...
    Synthetic,
    /// A reproduction of a documented historical decoder/renderer bug
    /// ([`Family::RealBug`], see [`real_bugs`]).
    RealBug {
        /// `real-bug:<repo>#<ref>`, from [`RealBugId::source_reference`].
        reference: String,
    },
}
...
/// The gold-standard members: one whole-frame, opaque entry per documented
/// real bug ([`RealBugId::all`]). Real bugs are not swept over region or
/// severity — each *is* its own extent and magnitude.
pub fn real_bug_catalog() -> Vec<CorruptionParams> {
    RealBugId::all()
        .iter()
        .map(|&id| CorruptionParams {
            family: Family::RealBug(id),
            region: Region::Whole,
            severity: Severity::Opaque,
        })
        .collect()
...
/// [`catalog`] (the ten synthetic families × region × severity) followed by
/// [`real_bug_catalog`]: everything a reference is swept with.
pub fn full_catalog() -> Vec<CorruptionParams> {
    let mut all = catalog();
    all.extend(real_bug_catalog());
    all
}

/// Build the synthetic catalog of [`CorruptionParams`] for one reference:
/// every synthetic family variant × the region sweep × the severity sweep.
/// Edge families that act on the whole frame use only `Region::Whole`. Real
/// bugs are not included; see [`full_catalog`].
...
/// Build manifest entries for one reference across the [`full_catalog`]:
/// the synthetic sweep (`source: synthetic`) followed by the real-bug
/// gold-standard members (`source: real-bug:<repo>#<ref>`).
///
...
pub fn manifest_for_reference(
    ...
    full_catalog()
        ...
        .map(|params| {
            ...
            let source = match params.family {
                Family::RealBug(id) => EntrySource::RealBug {
                    reference: id.source_reference(),
                },
                _ => EntrySource::Synthetic,
            };
            ManifestEntry {
...
mod tests {
    ...
    #[test]
    fn real_bug_catalog_is_whole_frame_opaque_and_in_the_manifest() {
        let rb = real_bug_catalog();
        assert_eq!(rb.len(), RealBugId::all().len());
        for p in &rb {
            assert_eq!(p.region, Region::Whole);
            assert_eq!(p.severity, Severity::Opaque);
            assert_eq!(p.family.name(), "real_bug");
            assert!(is_whole_frame_family(&p.family));
        }
        assert_eq!(full_catalog().len(), catalog().len() + rb.len());
        assert!(
            !catalog()
                .iter()
                .any(|p| matches!(p.family, Family::RealBug(_))),
            "the synthetic catalog stays synthetic"
        );

        let m = manifest_for_reference("photo/01", ContentClass::Photo, 7);
        let real: Vec<&ManifestEntry> = m
            .iter()
            .filter(|e| matches!(e.source, EntrySource::RealBug { .. }))
            .collect();
        assert_eq!(real.len(), RealBugId::all().len());
        assert!(real.iter().all(|e| e.family_name == "real_bug"));
        assert!(
            real.iter().any(|e| matches!(&e.source,
                EntrySource::RealBug { reference } if reference == "real-bug:zenjpeg#bd0f8d7")),
            "the chroma-boundary analogue row is present"
        );
        // Egregious ones expect a negative score, subtle ones only rank below lq.
        assert!(real.iter().any(|e| e.params.family
            == Family::RealBug(RealBugId::ZenjpegXybDoubleScaleWhite)
            && e.expected_negative));
        assert!(real.iter().any(|e| e.params.family
            == Family::RealBug(RealBugId::ZenwebpUvRoundingOffByOne)
            && !e.expected_negative));
        // Serde: the tag + id survive a round trip.
        let entry = real[0];
        let json = serde_json::to_string(entry).unwrap();
        assert!(json.contains("\"family\":\"real_bug\""), "{json}");
        let back: ManifestEntry = serde_json::from_str(&json).unwrap();
