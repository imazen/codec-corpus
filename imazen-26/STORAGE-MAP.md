...
- Raw GPS coordinates are stripped, but **location is preserved as landmark names baked
  into the filenames** by `add_gps_locations.py` + `add_landmarks.py` (geocode /
  nominatim / overpass caches in the archive).
...
`PROVENANCE.md` in this folder (and the copy at `s3://codec-corpus/imazen-26-unprocessed/PROVENANCE.md`)
describes folders (`office-documents/`, `nasa/`, `noaa/`, `national-park-service/`,
`internet-archive-scans/`) that do not match the current 21-category structure
(`1000-lilith-photos-general` ... `9226-lilith-ai-products`) listed above. It appears to be
carried over from an earlier corpus iteration and was not updated when the corpus was
restructured. Flagged 2026-07-22, not corrected — content ownership/history unclear from
this doc alone. The per-folder `PROVENANCE.md` files and `CORPUS-MANIFEST.tsv` match the
live structure and are the reliable source for provenance.
